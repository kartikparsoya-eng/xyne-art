#!/usr/bin/env bash
# validate-g8fix.sh — stand up the G8-fix rust image as a fresh syncer, run the
...
IMAGE="${IMAGE:-zero-cache-rust-syncer:g8fix}"
NAME="${NAME:-xyne-sandbox-rust-test-zero-cache-g8fix}"
NET="${NET:-sandbox-net}"
VOL="${VOL:-g8fix_zero}"
...
echo "== (re)create g8fix syncer container from $IMAGE =="
